...
public class SimulationSettingsViewModel implements ViewModel, IStorableGuiData {
	...
	private Set<Class<?>> getClassesFromJarFile(File jarFile) {
		...
		try {
			...
			Util.showError("Error loading Jar File", MessageFormat.format(
					"Error loading the file: {0}\n\nReason:\nThe file could not be found!", jarFile.getAbsoluteFile()));
		} catch (IOException e) {
			Util.showError("Error loading Jar File",
					MessageFormat.format("Error loading the file: {0}", jarFile.getAbsoluteFile()));
		}
		return classNames;
	}
