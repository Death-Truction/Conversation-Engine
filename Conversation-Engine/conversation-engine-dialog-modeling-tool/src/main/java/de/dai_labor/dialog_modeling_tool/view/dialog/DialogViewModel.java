...
/**
 * The ViewModel for the {@link DialogView}
 *
 * @author Marcel Engelmann
 *
 */
@Singleton
public class DialogViewModel implements ViewModel, IStorableGuiData {
	...
	private final Pane dialogElementsLayer = new Pane();
	...
	/**
	 * Create a new {@link DialogViewModel} instance
	 *
	 * @param settings The instance of the {@link Settings} class
	 */
	public DialogViewModel(Settings settings) {
		this.settings = settings;
		this.dialogPane = new DialogPane(this.dialogElementsLayer, this.insertMode, this.selectedState,
				this.selectedTransition, this::addState, this::addTransition, this::removeState,
				this::removeTransition);
		Platform.runLater(() -> {
			this.dialogElementsLayer.setMinWidth(DIAGRAM_ELEMENT_LAYER_SIZE);
			this.dialogElementsLayer.setMinHeight(DIAGRAM_ELEMENT_LAYER_SIZE);
			this.dialogElementsLayer.relocate(DIAGRAM_ELEMENT_LAYER_SIZE / -2.0, DIAGRAM_ELEMENT_LAYER_SIZE / -2.0);
		});
		this.dialogPane.toBack();
		this.subscribe("unload", (ignore, ignore2) -> this.dialogPane.deselectAll());
		this.dialogPane.toBack();
		this.viewProperty.set(this.dialogPane);
	...
	/**
	 * Gets the {@link DialogPane} that is displayed at the center of the view
	 *
	 * @return the {@link DialogPane} that is displayed at the center of the view
	 */
	...
	/**
	 * Centers the children of the {@link DialogPane} in the current viewport
	 */
	public void centerElements() {
		this.dialogPane.center();
	}
	...
	public void addState(int id, String name, double locationX, double locationY, boolean focusRequest) {
		...
		this.dialogElementsLayer.getChildren().add(newState);
	...
	public void removeState(State state) {
		for (final Iterator<Transition> iterator = this.transitions.iterator(); iterator.hasNext();) {
			...
			if (transition.getSource().equals(state) || transition.getTarget().equals(state)) {
				iterator.remove();
				this.dialogElementsLayer.getChildren().remove(transition);
			}
		}
		this.dialogElementsLayer.getChildren().remove(this.states.remove(state.getStateId()));
	...
	public void addTransition(State source, State target, String trigger, boolean focusRequest) {
		if (!this.transitionExists(source, target)) {
			...
			this.dataHasChanged = true;
			this.dialogElementsLayer.getChildren().add(newTransition);
	...
	public void resetData() {
		this.dialogElementsLayer.getChildren().clear();
	...
	@Override
	public void setGUIData(JSONObject dialogModelDataObject) {
		this.resetData();
		try {
			final JSONArray newStates = dialogModelDataObject.getJSONArray("states");
			final JSONArray newTransitions = dialogModelDataObject.getJSONArray("transitions");
			final double locationX = dialogModelDataObject.optDouble("locationX", -5000);
			final double locationY = dialogModelDataObject.optDouble("locationY", -5000);
			final double scale = dialogModelDataObject.optDouble("scale", 1.0);
			this.setViewScale(1);
			this.dialogElementsLayer.setTranslateX(0);
			this.dialogElementsLayer.setTranslateY(0);
			this.dialogElementsLayer.relocate(locationX, locationY);
	...
	public JSONObject getGUIData() {
		...
		final double scale = this.dialogElementsLayer.getScaleX();
		this.setViewScale(1);
		data.put("states", this.getStatesGUIData());
		data.put("transitions", this.getTransitionsGUIData());
		data.put("locationX", this.dialogElementsLayer.getBoundsInParent().getMinX());
		data.put("locationY", this.dialogElementsLayer.getBoundsInParent().getMinY());
	...
	private void setViewScale(double scale) {
		this.dialogElementsLayer.setScaleX(scale);
		this.dialogElementsLayer.setScaleY(scale);
	}
}